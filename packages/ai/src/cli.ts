...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @eureka/ai <command> [provider]
...
Examples:
  npx @eureka/ai login              # interactive provider selection
  npx @eureka/ai login anthropic    # login to specific provider
  npx @eureka/ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @eureka/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @eureka/ai --help' for usage`);
	process.exit(1);
}
